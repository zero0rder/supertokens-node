    ...
    let obj = {
        createNewSession: function ({ res, userId, accessTokenPayload = {}, sessionData = {} }) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                let payload = Object.assign({}, accessTokenPayload);
                for (const grant of config.defaultRequiredGrants) {
                    const value = yield grant.checkUser(userId);
                    if (value !== undefined) {
                        payload = grant.addToAccessTokenPayload(payload, value);
                    }
                }
                const response = yield SessionFunctions.createNewSession(helpers, userId, payload, sessionData);
